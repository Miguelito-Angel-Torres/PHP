...
/*'Coming Soon','Release','In Issue','Finish','Canceled'*/
-- Tabla Catalogo:
CREATE TABLE statu(
...
CREATE TABLE movieseries(
    ...
    plot TEXT,
    -- DEFAULT: INDICA SI ESTE CAMBO MANDA VACIO EL USUARIO ESCRIBE LO SIGUIENTE:
    ...
    mose_statu INTEGER UNSIGNED NOT NULL,
    -- ENUM: NOS INDICA UNA LISTA (CATALOGO) RECOMENDADO HACER QUE NO PASE MAS DE 10.
    category ENUM('Movie','Serie') NOT NULL,
    FULLTEXT KEY search(title,author,actors,genres),
    constraint FK_statu_movieseries
    FOREIGN KEY (mose_statu)
    references statu(status_id)
    /*RESTRICT: NO SE VA PODER ELIMINAR EL REGISTRO DE LA TABLA "STATUS" PORQUE TIENE RELACION "PELICULA" EN EL CAMPO RELACIONADO
    HASTA QUE LA TABLA "PELICULA" NO TENGA RELACION SI SE PODRA ELIMINARLO EL REGISTRO.
    CASCADE:SI HACES CAMBIO DE REGISTRO EN LA TABLA "STATUS" VA SER TABMIEN REFREJADO EN LA TABLA "PELICULA" EN EL CAMPO RELACIONADO*/
    ON DELETE RESTRICT ON UPDATE CASCADE
);

CREATE TABLE users(
	user_ VARCHAR(20) PRIMARY KEY,
    email VARCHAR(80) UNIQUE NOT NULL,
    name_ VARCHAR(100) NOT NULL,
    birthday DATE NOT NULL,
    pass CHAR(32) NOT NULL,
    role_ ENUM('Admin','User')
);
...
INSERT INTO users(user_,email,name_,birthday,pass,role_) VALUES
	('@miki','miguel@gmail.com','ML','2022-01-15',md5('MLX'),'Admin'),
    ('@lucas','lucas@gmail.com','LUC','2022-05-15',md5('LUCX'),'User');
    
INSERT INTO movieseries(imdb_id,title,plot,genres,author,actors,country,premiere,trailer,poster,rating,mose_statu,category) VALUES
